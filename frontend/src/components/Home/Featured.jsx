...
const Featured = () => {
  ...
  const playstationItem = ITEMS.find((item) => item.title === "PlayStation 5");
  const womenCollectionsItem = ITEMS.find(
    (item) => item.title === "Women's Collections"
  );
  const speakersItem = ITEMS.find((item) => item.title === "Speaker");
  const perfumesItem = ITEMS.find((item) => item.title === "perfume");

  return (
    <div className="flex flex-col my-24">
      <div className="mx-2">
        <RedTitle title="Featured" />
        <h2 className="text-2xl md:text-3xl font-semibold mb-14">Featured</h2>
      </div>
